    ...
    async def _run_one(task_def: dict) -> dict:
        ...
        try:
            return await run_browser_agent(
                ...
                # an explicit url means "go here" even on the user's picked card; with none, a picked card stays on the page they parked it. app_mode never navigates.
                initial_url=None if app_mode else (_nav_url if _nav_url and (url or browser_id not in pre_selected) else None),
                parent_session_id=parent_session_id,
                app_mode=app_mode,
            )
        finally:
